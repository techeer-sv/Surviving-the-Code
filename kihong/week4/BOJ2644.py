...
while stack:
    ...
    만약 부모를 방문하지 않았을 경우 부모 노드 방문
        continue
시간 복잡도는 O(n^2) 즉, O(n^2)이다. => while 문 내에 부모를 찾는 O(n)의 시간 복잡도가 있으므로 O(n)*O(n)의 시간 복잡도를 지님.

해당 문제는 DFS로 풀었기 때문에 다음과 같은 결과가 나왔다. 만약 BFS로 풀었다면 시간 복잡도는 O(n)이 나오게 될 것이다.
"""
